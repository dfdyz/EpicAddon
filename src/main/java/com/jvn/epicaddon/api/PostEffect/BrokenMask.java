...
import static com.jvn.epicaddon.register.RegPostEffect.depthBuffer;
...
public class BrokenMask extends PostEffectBase{
    ...
    public void process(RenderTarget inTarget, RenderTarget outTarget, float time, PostEffectUtils.OBJ_JSON obj, int a, float ang, float[] pos){
        ...
        float sss = Math.max(inTarget.height, inTarget.width)/6f;
        ...
        for(int index = 0; index < obj.Face.size(); ++index) {
            ...
            v1.add(inTarget.width/2.0f, inTarget.height*0.58f,500);
            v2.add(inTarget.width/2.0f, inTarget.height*0.58f,500);
            v3.add(inTarget.width/2.0f, inTarget.height*0.58f,500);
