...
public class GenShinBowLandingParticle extends TextureSheetParticle {
    ...
    protected GenShinBowLandingParticle(ClientLevel level, double x, double y, double z, double xd, double yd, double zd, int lifeTick) {
        ...
        level.addParticle(RegParticle.GENSHIN_BOW_LANDING2.get(), this.x,this.y,this.z+0.3,30,0,0);
        level.addParticle(RegParticle.GENSHIN_BOW_LANDING2.get(), this.x+0.5,this.y,this.z,20,90,0);
        level.addParticle(RegParticle.GENSHIN_BOW_LANDING2.get(), this.x-0.35,this.y,this.z-0.35,25,-135,0);

        for (int i = 0; i < 9; i++) {
            double offd = random.nextDouble(0,2.5);
            double offr = random.nextDouble(0,360);
            int offt = random.nextInt(2,8);
            level.addParticle(RegParticle.GENSHIN_BOW_LANDING3.get(),this.x+Math.cos(offr)*offd, this.y, this.z-Math.sin(offr)*offd, offt,0,0);
        }
    }

    @Override
    public ParticleRenderType getRenderType() {
        return EpicAddonRenderType.GENSHIN_BOW_LANDING_PARTICLE;
    }
